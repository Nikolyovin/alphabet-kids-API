...
app.get('/letters/:id', (req: RequestWithParams<URIParamsLetterIdModel>, res: Response<LetterViewModel>) => {
...
app.post('/letters', async (req: RequestWithBody<LetterCreateModel>, res: Response<LetterCreateModel>) => {
    try {
        ...
        const letter = await Letter.create({ name, voice, image, words })
        res.status(HTTP_STATUSES.CREATED_201).json(letter)
    } catch (e) {
        console.log(e)
        res.status(500)
    }
...
app.put(
    '/letters/:id',
    (req: RequestWithParamsAndBody<URIParamsLetterIdModel, LetterCreateModel>, res: Response<LetterUpdateModel>) => {
